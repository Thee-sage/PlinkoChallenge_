...
export class BallManager {
    ...
    private setCanvasSize() {
        ...
        if (isMobile) {
            // Mobile-specific handling with DPR
            const canvasWidth = WIDTH * this.scale;
            const canvasHeight = HEIGHT * this.scale;
            
            this.canvasRef.width = Math.round(canvasWidth * dpr);
            this.canvasRef.height = Math.round(canvasHeight * dpr);
            
            this.canvasRef.style.width = `${canvasWidth}px`;
            this.canvasRef.style.height = `${canvasHeight}px`;
            
            this.ctx.scale(dpr * this.scale, dpr * this.scale);
            this.ctx.imageSmoothingEnabled = true;
            this.ctx.imageSmoothingQuality = 'high';
        } else {
            // Desktop/Laptop handling - keep original behavior
